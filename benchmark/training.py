...
CONFIG = {
    ...
    'MEMORY_INTERVAL': 0.1,
    'TOP_K_CLASSES': 100,
    'BASE_DIR': '/kaggle/working/rki_2025/model',
...
class EnhancedDataLoader:
    ...
    - Filter berdasarkan level taxonomi, kmer length, dan devices
    ...
    def filter_reduction_data(self, level=None, kmer=None, file_types=None, devices=None, methods=None):
        """
        Filter reduction data by level, kmer, file types, devices, AND methods
        
        Parameters:
        -----------
        level : str, optional
            Taxonomic level (e.g., 'class', 'family', 'genus')
        kmer : str, optional  
            K-mer length (e.g., 'k6', 'k8', 'k10')
        file_types : list, optional
            List of file types to include (e.g., ['features_train', 'features_test'])
        devices : list, optional
            List of devices to filter (e.g., ['cpu', 'gpu'])
        methods : list, optional
            List of reduction methods to filter (e.g., ['ipca', 'umap', 'svd'])
        
        Returns:
        --------
        list : Filtered reduction data entries
        """
        filtered = self.reduction_data
        ...
        if devices:
            ...
            filtered = [item for item in filtered 
                       if item['device'].lower() in devices_lower]
        
        # 🆕 NEW: Filter by methods
        if methods:
            methods_lower = [m.lower() for m in methods]
            filtered = [item for item in filtered 
                       if item['method'].lower() in methods_lower]
        ...
        for path in self.vectorization_paths:
            ...
            if level_match and kmer_match and prefix_match:
                ...
                filtered_paths.append({
                    ...
                    'kmer': extracted_kmer,
                    'filename': filename,
                    'file_type': filename.split('_')[0] + '_' + filename.split('_')[1]
                })
    ...
    def load_dataset_for_pipeline(self, level, kmer, devices=None, methods=None):
        """
        Load complete dataset for ML pipeline with device and method filters
        """
        print(f"\n🔍 Loading dataset: {level} × {kmer}")
        if devices:
            print(f"🖥️  Device filter: {devices}")
        if methods:
            print(f"🔬 Method filter: {methods}")
        
        try:
            # Filter reduction data with devices AND methods
            reduction_data = self.filter_reduction_data(
                level=level, 
                kmer=kmer, 
            ...
            if not reduction_data:
                print(f"❌ No reduction data found for {level} × {kmer}")
                return None
            
            print(f"📊 Found {len(reduction_data)} reduction files after filtering")
            
            # 🔍 DEBUG: Print all files found
            print(f"\n🔍 DEBUG: All reduction files found:")
            for item in reduction_data:
                print(f"   - Type: {item['file_type']}")
                print(f"     File: {os.path.basename(item['file_path'])}")
                print(f"     Method: {item.get('method', 'unknown')}")
                print(f"     Device: {item.get('device', 'unknown')}")
            
            # Determine strategy based on available files
            train_files = [item for item in reduction_data if item['file_type'] == 'features_train']
            test_files = [item for item in reduction_data if item['file_type'] == 'features_test']
            
            print(f"\n📊 Files breakdown:")
            print(f"   - Train files: {len(train_files)}")
            print(f"   - Test files: {len(test_files)}")
            
            # 🔍 DEBUG: Print test files specifically
            if test_files:
                print(f"\n✅ Test files found:")
                ...
                print(f"\n❌ NO TEST FILES FOUND!")
                print(f"🔍 Checking what's in reduction_data:")
                unique_types = set([item['file_type'] for item in reduction_data])
                print(f"   Unique file_types: {unique_types}")
            ...
            if not test_files:
                print(f"❌ No test files found - THIS IS THE PROBLEM!")
                return None
            
            if len(train_files) > 1 or len(test_files) > 1:
                strategy = 'smart_aggregate'
                print(f"📊 Strategy: {strategy} (multiple files detected)")
            else:
            ...
            if strategy == 'smart_aggregate':
                print(f"🔄 Executing smart aggregation...")
                ...
                for item in reduction_data:
                    ...
                    method_name = item.get('method', '').lower()
                    
                    if 'umap' in method_name or 'umap' in file_path.lower():
                        method_groups['umap'].append(item)
                    elif 'ipca' in method_name or 'ipca' in file_path.lower() or 'pca' in file_path.lower():
                        method_groups['ipca'].append(item)
                    elif 'svd' in method_name or 'svd' in file_path.lower():
                        method_groups['svd'].append(item)
                    else:
                        method_groups['other'].append(item)
                
                # Select best method (most files)
                ...
                if best_method and method_groups[best_method]:
                    ...
                    if train_dfs:
                        ...
                        component_cols = sorted([col for col in common_cols if col.startswith('component_')])
                        metadata_cols = [col for col in ['label', 'method', 'device', 'fold'] if col in common_cols]
                        
                        print(f"🔄 Using {len(component_cols)} common component features")
                        
                        aligned_dfs = [df[component_cols + metadata_cols] for df in train_dfs]
                    ...
                    if test_dfs:
                        if component_cols:
                            ...
                            for df in test_dfs:
                                ...
                                metadata_cols_test = [col for col in ['label', 'method', 'device', 'fold'] if col in df.columns]
                                aligned_test_dfs.append(df[available_cols + metadata_cols_test])
                        ...
                        print(f"❌ No test dataframes were loaded!")
                    
            else:  # strategy == 'single_file'
                print(f"📄 Loading single files...")
            ...
            # Load vectorization labels
            print(f"🔍 Loading vectorization labels...")
            vectorization_filtered = self.filter_vectorization_data(
                level=level,
                kmer=kmer,
            ...
            if missing_components:
                ...
                print(f"   Total reduction files: {len(reduction_data)}")
                return None
            
            # Validate dimensions
            if len(result['X_train_reduction']) != len(result['y_train']):
                print(f"⚠️ Dimension mismatch: X_train({len(result['X_train_reduction'])}) vs y_train({len(result['y_train'])})")
                return None
            
            if len(result['X_test_reduction']) != len(result['y_test']):
                print(f"⚠️ Dimension mismatch: X_test({len(result['X_test_reduction'])}) vs y_test({len(result['y_test'])})")
                return None
            ...
            if train_component_cols and test_component_cols:
                ...
                print(f"🔄 Feature alignment: {len(train_component_cols)} → {min_features}")
            ...
            print(f"\n✅ Successfully loaded dataset: {level} × {kmer}")
            print(f"   📊 Strategy: {strategy}")
            if devices:
                print(f"   🖥️  Devices: {devices}")
            if methods:
                print(f"   🔬 Methods: {methods}")
            print(f"   📐 X_train: {result['X_train_reduction'].shape}")
            print(f"   📐 X_test: {result['X_test_reduction'].shape}")
            print(f"   🏷️  y_train: {result['y_train'].shape}")
            print(f"   🏷️  y_test: {result['y_test'].shape}")
    ...
    def batch_load_all_datasets(self, levels=None, kmers=None, devices=None):
        """
        ...
        Args:
            ...
            devices: List devices yang ingin di-load (None = semua)
        ...
        for level, kmer in available_datasets:
            ...
            dataset = self.load_dataset_for_pipeline(level, kmer, devices=devices)
...
# ...existing code for EarlyStopping, TrainingMonitor, EnhancedMLPipeline classes...
...
def run_enhanced_pipeline_with_enhanced_loader(level, kmer, 
                                               reduction_log_path,
                                               vectorization_paths_file,
                                               devices=None,
                                               methods=None):  # 🆕 NEW parameter
    """
    Run enhanced pipeline with EnhancedDataLoader integration
    ...
    kmer : str
        K-mer length (e.g., 'k6', 'k8', 'k10')
    reduction_log_path : str
        Path to reduction log file
    vectorization_paths_file : str
        Path to vectorization paths file
    devices : list, optional
        Filter by device types (e.g., ['cpu', 'gpu'])
    methods : list, optional
        Filter by reduction methods (e.g., ['ipca', 'umap', 'svd'])
    
    Returns:
    --------
    tuple : (results_df, best_model_name, metadata)
    """
    print(f"\n🚀 **RUNNING ENHANCED PIPELINE WITH ENHANCED LOADER**")
    print(f"📊 Dataset: {level} × {kmer}")
    if devices:
        print(f"🖥️  Device filter: {devices}")
    if methods:
        print(f"🔬 Method filter: {methods}")
    ...
    try:
        ...
        dataset = loader.load_dataset_for_pipeline(
            level=level, 
            kmer=kmer,
            devices=devices,
            methods=methods  # 🆕 NEW: Pass methods filter
        )
        ...
        print(f"   - Unique classes: {len(np.unique(y_train))}")
        if devices:
            print(f"   🖥️  Device filter applied: {devices}")
...
def batch_process_with_enhanced_loader(reduction_log_path, vectorization_paths_file, 
                                     levels=None, kmers=None, devices=None):
    """
    ...
    Args:
        ...
        devices: List devices yang ingin di-process (None = semua)
    ...
    for i, (level, kmer) in enumerate(target_datasets, 1):
        ...
        results_df, best_model, metadata = run_enhanced_pipeline_with_enhanced_loader(
            level, kmer, reduction_log_path, vectorization_paths_file, devices=devices
        )
        end_time = time.time()
